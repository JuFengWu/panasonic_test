...
void csp_move_back_and_forth(uint16 slave, double posA_deg, double posB_deg, int cycles)
{
    ...
    for (int c = 0; c < cycles; c++)
    {
        ...
        {
            csp_set_target_position(slave, cmd);
            cmd += step;
            usleep(dt_us);
        }
        csp_set_target_position(slave, posB);
        ...
        csp_set_target_position(slave, posA);
...
int main(int argc, char *argv[])
{
    ...
    csp_move_back_and_forth(SLAVE_ID, 0.0, 30.0, 5);
