...
int main()
{
  ...
  const char* ifname = "enp3s0";    // Network interface name
  ...
  // Step 2: Use callback to read current position (no control in callback)
  sys.session().setCallback(read_position_callback);

  // Step 3: Start background loop (control loop enabled)
  if (!sys.run_async()) {
